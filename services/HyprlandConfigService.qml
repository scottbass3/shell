...
// SettingsService, get generated into ~/.local/state/scottbass3-shell/hypr.generated.lua,
...
// black out a screen).
//
// Also owns the "launch in special workspace" window rules (hypr.specialRules)
// edited from Settings → Tray.
QtObject {
    id: root
    ...
    function refresh() { _probe.running = false; _probe.running = true; _monProbe.running = false; _monProbe.running = true; refreshClients() }
    function refreshClients() { _cliProbe.running = false; _cliProbe.running = true }
    ...
    property var clients: []         // [{ class, title, workspace }] of open windows

    property Process _cliProbe: Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                try { arr = JSON.parse(text) } catch (e) { return }
                if (!Array.isArray(arr)) return
                root.clients = arr.filter(c => (c.class ?? "") !== "")
                                  .map(c => ({ class: c.class, title: c.title ?? "", workspace: c.workspace?.name ?? "" }))
            }
        }
    }
    ...
    // ── Special-workspace launch rules ─────────────────────────────────────────
    // [{ class, ws, enabled }]. Disabled entries are kept (not emitted) so the
    // toggle round-trips without retyping the class regex.
    readonly property var specialRules: SettingsService.get("hypr.specialRules", [])

    function setSpecialRules(a) { SettingsService.set("hypr.specialRules", a); applyLive() }
    function addSpecialRule(cls, ws) {
        const a = specialRules.slice()
        a.push({ class: cls ?? "", ws: ws ?? "", enabled: true })
        setSpecialRules(a)
    }
    function setSpecialRule(i, field, val) {
        const a = specialRules.slice()
        if (i < 0 || i >= a.length) return
        const e = Object.assign({}, a[i]); e[field] = val; a[i] = e
        setSpecialRules(a)
    }
    function removeSpecialRule(i) {
        const a = specialRules.slice()
        if (i < 0 || i >= a.length) return
        a.splice(i, 1)
        setSpecialRules(a)
    ...
    // First open-window class the pattern matches ("" = none, null = invalid
    // regex). Hyprland full-matches with RE2; JS RegExp agrees on the plain
    // patterns window rules use.
    function matchedClass(pattern) {
        void root.clients
        let re
        try { re = new RegExp("^(?:" + pattern + ")$") } catch (e) { return null }
        for (const c of clients) if (re.test(c.class)) return c.class
        return ""
    }

    function _specialRulesLua() {
        const out = []
        specialRules.forEach((r, i) => {
            const cls = String(r.class ?? "").trim(), ws = String(r.ws ?? "").trim()
            if (r.enabled === false || cls === "" || ws === "") return
            out.push("hl.window_rule({ name = " + _luaLeaf("str", "qs-special-" + i)
                   + ", match = { class = " + _luaLeaf("str", cls) + " }, workspace = "
                   + _luaLeaf("str", "special:" + ws + " silent") + " })")
        })
        return out
    }
